<template>
  <div ref="container" id="picture-input" class="picture-input mb-3">
    ...
    <div v-else-if="supportsPreview">
      ...
      <button class="mt-3" v-if="imageSelected && !hideChangeButton" @click.prevent="selectImage" :class="buttonClass" type="button">{{ strings.change }}</button>
      <button class="mt-3" v-if="imageSelected && removable" @click.prevent="removeImage" :class="removeButtonClass" type="button">{{ strings.remove }}</button>
      <button class="mt-3" v-if="imageSelected && toggleAspectRatio && width !== height" @click.prevent="rotateImage" :class="aspectButtonClass" type="button">{{ strings.aspect }}</button>
    </div>
    <div v-else>
